...
function FormFieldInput({
  ...
  required = false, disabled = false, error, value, onChange,
}: {
...
function StatusMessage({ type, message, onDismiss }: {
  ...
  const styles = {
    success: { bg: "bg-green-900/20", border: "border-green-800", text: "text-green-300" },
    error:   { bg: "bg-red-900/20",   border: "border-red-800",   text: "text-red-300"   },
  };
  const s = styles[type];
  return (
    <div className={`flex items-start gap-3 p-4 rounded-lg border ${s.bg} ${s.border} ${s.text}`} role="alert">
...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email === "admin@example.com" && password === "password123") {
      ...
      // TODO: router.push("/admin/dashboard") or your auth logic here
    } else {
      setSubmitStatus("error");
      setStatusMsg("Invalid credentials. Please check your email and password.");
    }
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-slate-900/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-2xl border border-slate-800">
          ...
          {statusMsg && (
            <div className="mb-6">
              <StatusMessage
                ...
                message={statusMsg}
                onDismiss={() => { setStatusMsg(""); setSubmitStatus("idle"); }}
              />
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            {/* Password */}
            <div className="flex flex-col">
              <label htmlFor="password" className="text-slate-300 text-sm font-medium mb-2">
            ...
            <div className="flex items-center justify-between gap-4">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-slate-300 text-sm">Remember me for 30 days</span>
              </label>
              <a
                href="#"
