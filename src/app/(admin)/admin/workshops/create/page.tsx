...
import {
  ...
  CalendarDays, Clock, Timer, BadgeCheck, IndianRupee, Rocket,
} from "lucide-react";
...
function FormInput({
  name, label, placeholder, type = "text", icon,
  required, disabled, error, value, onChange,
}: {
...
function FormSelect({
  name, label, options, placeholder = "Select…",
  required, disabled, error, value, onChange,
}: {
    ...
    <div>
      ...
      <div className="relative">
        <select
          ...
          `}
        >
          <option value="" disabled className="text-zinc-500 bg-zinc-900">{placeholder}</option>
          {options.map((o) => (
            <option key={o.value} value={o.value} className="text-zinc-100 bg-zinc-900">
              {o.label}
            </option>
          ))}
        </select>
...
function FormToggle({
  name, label, helperText, checked, onChange,
}: {
  name: string; label: string; helperText?: string;
...
function FormFileUpload({
  name, label, accept, hint, icon, required, error, fileName, onFileSelect,
}: {
  name: string; label: string; accept?: string; hint?: string;
  ...
  return (
    <div>
      ...
      <div className={`relative border-2 border-dashed rounded-md transition-colors cursor-pointer group
        ${error
      ...
      `}>
        <input
          type="file" id={name} name={name} accept={accept}
          onChange={(e) => onFileSelect?.(e.target.files?.[0] || null)}
          className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
        />
        <div className="flex flex-col items-center gap-2 py-5 px-4 text-center">
          <span className={`${error ? "text-rose-400" : "text-zinc-500 group-hover:text-blue-400"} transition-colors`}>
            {icon}
          </span>
          <div>
            <p className="text-sm text-zinc-400 group-hover:text-zinc-300 transition-colors">
              {fileName
                ? <span className="text-green-400 font-medium block truncate max-w-[200px]">{fileName}</span>
...
export default function AddNewWorkshopPage() {
  const [form, setForm] = useState<Omit<WorkshopPayload, "thumbnail" | "certificate_template">>({
    title: "",
    description: "",
    eligibility: "",
    mode: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const endpoint of candidateEndpoints) {
        ...
        if (multipartRes.status === 404) {
          continue;
        }

        // Fallback for APIs that accept base64 JSON instead of multipart.
        const jsonRes = await fetch(endpoint, {
          method: "POST",
          headers: jsonHeaders,
          credentials: "include",
        ...
        let jsonData: any = {};
        try {
          jsonData = jsonRaw ? JSON.parse(jsonRaw) : {};
        } catch {
          jsonData = {};
        }
        ...
        throw new Error(
          ...
          jsonData?.detail ||
          jsonData?.message ||
          `Error ${multipartRes.status || jsonRes.status}`,
        );
      }

      if (!success) {
        throw new Error(
          `Create workshop endpoint not found (404). Tried: ${candidateEndpoints.join(", ")}`,
        );
      }
  ...
  const clearForm = () => {
    setForm({
      title: "", description: "", eligibility: "", mode: "",
      workshop_date: "", start_time: "", end_time: "",
      duration: "", certificate: false, fee: 0,
    });
  ...
  return (
    <div className="min-h-screen text-zinc-100 container mx-auto max-w-8xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-3 pb-5 mb-6 border-b border-zinc-800">
        <div>
          <h1 className="text-4xl font-bold tracking-tight text-white">Add New Workshop</h1>
          <p className="text-sm text-zinc-400 mt-1">Fill in the details to publish a new workshop</p>
        </div>
      ...
      {apiError && (
        <div className="mb-6 flex items-start gap-3 px-4 py-3 rounded-md bg-rose-950/50 border border-rose-800 text-rose-300 text-sm">
          ...
          <button onClick={() => setApiError("")} className="hover:text-rose-100 transition-colors">✕</button>
        </div>
      )}
            ...
            <Card className="bg-zinc-900 border-zinc-800">
              ...
              <CardContent className="space-y-5">
                ...
                <FormToggle
                  ...
                  onChange={(val) => setForm((f) => ({ ...f, certificate: val }))}
                />
